...
from dyn.models.elements import Element, TrajectoryElement, TrajFireworkElement, Position
...
class TrajBase(_FormBase):
	...
	def _setup_color(self) -> None:
		...
		self._color_start.color_changed.connect(lambda c: self._emit("traj_color_start", c))
		self._color_end.color_changed.connect(lambda c: self._emit("traj_color_end", c))
