...
class EffectBase(_FormBase):
	"""df 效果表单共享基类 子类只需实现 _setup_type_params 和 _load_type_params."""
	...
	def __init__(self, parent: QWidget | None = None) -> None:
		...
		self._element: EffectElement | None = None
	...
	def _setup_position(self) -> None:
		...
		self._btn_pos_select.clicked.connect(lambda: self.position_select_requested.emit("effect"))
	...
	def load(self, elem: EffectElement) -> None:
		self._loading = True
		self._element = elem
	...
	def _load_type_params(self, elem: EffectElement) -> None:
		"""子类重写以加载类型专属数据."""
