...
from dyn.models.elements import (
    ...
    GradientColor, ColorRGB, Position,
)
...
class _TFProxy(Element):
    ...
    def to_json(self) -> dict: return self._parent.to_json()
...
class _TrackArea(QWidget):
    ...
    def _paint_block(self, p: QPainter, blk: _BlockLayout) -> None:
        ...
        if sel:
            c = self._block_color(elem).lighter(140); c.setAlpha(230)
        ...
        if r.width() > 36:
            ...
            f2 = QFont(); f2.setPointSize(6); p.setFont(f2)
    ...
    def _paint_block_gradient(self, p: QPainter, elem: Element, r: QRect, sel: bool) -> None:
        ...
        if isinstance(elem, _TFProxy):
            ...
            if elem._part == "traj":
                ...
                c_start = parent.inner_color.start; c_end = parent.inner_color.end
                c_outer_start = parent.outer_color.start; c_outer_end = parent.outer_color.end
        elif isinstance(elem, TrajectoryElement):
            c_start, c_end = elem.traj_color.start, elem.traj_color.end
            use_grad = elem.traj_color.use_gradient; layers = 1
        elif isinstance(elem, FireworkElement):
            use_grad = elem.inner_color.use_gradient
            layers = 2 if elem.fw_type == "double_layer" else 1
            c_start = elem.inner_color.start; c_end = elem.inner_color.end
            c_outer_start = elem.outer_color.start; c_outer_end = elem.outer_color.end
        else:
            c_start = c_end = ColorRGB(128, 128, 128)
            use_grad = False; layers = 1
        ...
        p.setClipRect(r)
        if layers == 2:
            half_w = r.width() // 2; half_h = r.height() // 2
            in_grad = None; out_grad = None
            if isinstance(elem, _TFProxy):
            ...
            if out_grad:
                ...
                p.fillRect(QRect(r.x() + half_w, r.y() + half_h, r.width() - half_w, r.height() - half_h), QBrush(qc(c_outer_end)))
            else:
    ...
    def _icon(elem: Element) -> str:
        if isinstance(elem, _TFProxy):
            ...
            if elem._part == "traj":
                ...
                return {"single_layer": "✦", "double_layer": "✶", "directional": "➳", "clustered": "❈", "expanding_sphere": "◉"}.get(parent.fw_type, "★")
    ...
    def mouseMoveEvent(self, event: QMouseEvent) -> None:
        if self._dragging and self._drag_start_pos:
            ...
            self._compute(); self.update()
            self.drag_update.emit(); return
        # 光标样式
        x, y = event.position().x(), event.position().y()
        for blk in self._blocks:
            r = blk.rect
...
class TimelineWidget(QWidget):
    ...
    def seek_playback(self, tick: int) -> None:
        self._playback_tick = tick; self.update()
    ...
    def _paint_cursor(self, p: QPainter) -> None:
        ...
        if TRACK_LABEL_WIDTH <= cx <= self.width():
            ...
            path = QPainterPath()
            path.moveTo(cx - 6, 0); path.lineTo(cx + 6, 0); path.lineTo(cx, 8)
            path.closeSubpath()
    ...
    def mousePressEvent(self, event: QMouseEvent) -> None:
        if event.button() == Qt.MiddleButton:
            self._panning = True; self._pan_start_x = event.position().x()
    ...
    def mouseMoveEvent(self, event: QMouseEvent) -> None:
        ...
        if self._dragging_cursor:
            ...
            if x > self.width() - margin:
                ...
                self._refresh_tracks(); self.view_changed.emit()
            elif x < TRACK_LABEL_WIDTH + margin:
                self._scroll_offset = max(0, self._scroll_offset - (TRACK_LABEL_WIDTH + margin - x))
                self._refresh_tracks(); self.view_changed.emit()
            self.playback_cursor_changed.emit(self._playback_tick); self.update()
