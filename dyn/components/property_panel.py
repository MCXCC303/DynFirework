...
from PySide6.QtWidgets import (
    ...
    QGroupBox, QScrollArea, QFrame, QColorDialog, QDial,
)
...
class ColorPicker(QWidget):
    ...
    def _setup_ui(self, label: str) -> None:
        ...
        layout.setSpacing(4)
        if label:
            lbl = QLabel(label); lbl.setFixedWidth(40); layout.addWidget(lbl)
        self._btn = QPushButton(); self._btn.setFixedSize(52, 28)
        self._btn.setToolTip("点击选择颜色")
        self._btn.clicked.connect(self._on_pick_color)
        layout.addWidget(self._btn)
        for ch in ("R", "G", "B"):
            lbl = QLabel(ch); lbl.setAlignment(Qt.AlignRight | Qt.AlignVCenter); lbl.setFixedWidth(14)
            layout.addWidget(lbl)
            spin = QSpinBox(); spin.setRange(0, 255); spin.setFixedWidth(48)
            layout.addWidget(spin)
            if ch == "R": self._spin_r = spin
            elif ch == "G": self._spin_g = spin
            else: self._spin_b = spin
    ...
    def set_color(self, color: ColorRGB) -> None:
        self._color = color
        self._spin_r.setValue(color.r); self._spin_g.setValue(color.g); self._spin_b.setValue(color.b)
        self._update_button()
...
class PropertyPanel(QScrollArea):
    ...
    _TRAJ_TYPE_LABELS = {"launch": "基础发射轨迹", "spark": "火花轨迹", "offset": "随机偏移轨迹", "thick": "粗轨迹（偏移）", "expanding": "膨胀轨迹（偏移）"}
    _FW_TYPE_LABELS = {"single_layer": "单层烟花", "double_layer": "双层烟花", "directional": "定向烟花", "clustered": "集束烟花", "expanding_sphere": "膨胀球烟花"}
    ...
    def _setup_common_section(self) -> None:
        ...
        self._edit_name = QLineEdit(); self._edit_name.setPlaceholderText("元素名称")
        self._edit_name.textChanged.connect(lambda t: self._emit_change("name", t))
        self._add_row(form, "name", "名称:", self._edit_name)

        self._spin_start_tick = QSpinBox(); self._spin_start_tick.setRange(0, 100000)
        ...
        self._spin_duration = QSpinBox(); self._spin_duration.setRange(1, 10000)
        ...
        self._chk_enabled = QCheckBox("启用"); self._chk_enabled.setChecked(True)
    ...
    def _setup_position_section(self) -> None:
        ...
        self._spin_pos_x = QDoubleSpinBox(); self._spin_pos_x.setRange(-100000, 100000); self._spin_pos_x.setDecimals(2)
        self._add_row(form, "pos_x", "X:", self._spin_pos_x)
        self._spin_pos_y = QDoubleSpinBox(); self._spin_pos_y.setRange(-64, 320); self._spin_pos_y.setDecimals(2)
        self._add_row(form, "pos_y", "Y:", self._spin_pos_y)
        self._spin_pos_z = QDoubleSpinBox(); self._spin_pos_z.setRange(-100000, 100000); self._spin_pos_z.setDecimals(2)
        ...
        self._spin_end_x = QDoubleSpinBox(); self._spin_end_x.setRange(-100000, 100000); self._spin_end_x.setDecimals(2)
        self._add_row(end_form, "end_x", "X:", self._spin_end_x)
        self._spin_end_y = QDoubleSpinBox(); self._spin_end_y.setRange(-64, 320); self._spin_end_y.setDecimals(2)
        self._add_row(end_form, "end_y", "Y:", self._spin_end_y)
        self._spin_end_z = QDoubleSpinBox(); self._spin_end_z.setRange(-100000, 100000); self._spin_end_z.setDecimals(2)
    ...
    def _setup_traj_color_section(self) -> None:
        ...
        self._color_traj_start = ColorPicker("开始:"); layout.addWidget(self._color_traj_start)
        self._color_traj_end = ColorPicker("结束:"); layout.addWidget(self._color_traj_end)
    ...
    def _setup_traj_extra_section(self) -> None:
        ...
        self._spin_k = QDoubleSpinBox(); self._spin_k.setRange(0.1, 10.0); self._spin_k.setValue(1.2); self._spin_k.setSingleStep(0.1)
        self._add_row(form, "k", "阻力系数 k:", self._spin_k, 1.2)
        self._spin_m0 = QDoubleSpinBox(); self._spin_m0.setRange(0.1, 10.0); self._spin_m0.setValue(0.5); self._spin_m0.setSingleStep(0.1)
        self._add_row(form, "m0", "质量 m0:", self._spin_m0, 0.5)

        self._spin_rho = QSpinBox(); self._spin_rho.setRange(1, 100); self._spin_rho.setValue(1)
        self._add_row(form, "rho", "粒子密度 rho:", self._spin_rho, 1)
        self._spin_particle_count = QSpinBox(); self._spin_particle_count.setRange(1, 1000); self._spin_particle_count.setValue(1)
        self._add_row(form, "particle_count", "粒子数量:", self._spin_particle_count, 1)
        self._spin_interval = QSpinBox(); self._spin_interval.setRange(1, 100); self._spin_interval.setValue(5)
        self._add_row(form, "interval_ticks", "偏移间隔:", self._spin_interval, 5)
        self._spin_pts_per_tick = QSpinBox(); self._spin_pts_per_tick.setRange(1, 100); self._spin_pts_per_tick.setValue(1)
        self._add_row(form, "points_per_tick", "每 tick 点数:", self._spin_pts_per_tick, 1)
        self._spin_range_x = QDoubleSpinBox(); self._spin_range_x.setRange(0, 100); self._spin_range_x.setSingleStep(0.1)
        self._add_row(form, "range_x", "扩散 X:", self._spin_range_x, 0.0)
        self._spin_range_y = QDoubleSpinBox(); self._spin_range_y.setRange(0, 100); self._spin_range_y.setSingleStep(0.1)
        self._add_row(form, "range_y", "扩散 Y:", self._spin_range_y, 0.0)
        self._spin_range_z = QDoubleSpinBox(); self._spin_range_z.setRange(0, 100); self._spin_range_z.setSingleStep(0.1)
        self._add_row(form, "range_z", "扩散 Z:", self._spin_range_z, 0.0)
        self._spin_speed_factor = QDoubleSpinBox(); self._spin_speed_factor.setRange(0.1, 10.0); self._spin_speed_factor.setValue(1.0); self._spin_speed_factor.setSingleStep(0.1)
    ...
    def _setup_fw_color_inner(self) -> None:
        ...
        self._chk_inner_gradient = QCheckBox("使用渐变"); layout.addWidget(self._chk_inner_gradient)
        self._chk_inner_gradient.toggled.connect(lambda v: self._emit_change("inner_gradient", v))
        self._color_inner_start = ColorPicker("开始:"); layout.addWidget(self._color_inner_start)
        self._color_inner_end = ColorPicker("结束:"); layout.addWidget(self._color_inner_end)
    ...
    def _setup_fw_color_outer(self) -> None:
        ...
        self._chk_outer_gradient = QCheckBox("使用渐变"); layout.addWidget(self._chk_outer_gradient)
        self._chk_outer_gradient.toggled.connect(lambda v: self._emit_change("outer_gradient", v))
        self._color_outer_start = ColorPicker("开始:"); layout.addWidget(self._color_outer_start)
        self._color_outer_end = ColorPicker("结束:"); layout.addWidget(self._color_outer_end)
    ...
    def _setup_fw_angle_section(self) -> None:
        ...
        self._dial_h = QDial(); self._dial_h.setRange(1, 360); self._dial_h.setValue(30); self._dial_h.setNotchesVisible(True); self._dial_h.setFixedSize(80, 80)
        h_grp.addWidget(self._dial_h, alignment=Qt.AlignCenter)
        h_lbl = QLabel("水平角度"); h_lbl.setAlignment(Qt.AlignCenter)
        h_grp.addWidget(h_lbl)
        self._spin_h_angle = QDoubleSpinBox(); self._spin_h_angle.setRange(1, 360); self._spin_h_angle.setValue(30); self._spin_h_angle.setFixedWidth(80)
        ...
        self._dial_v = QDial(); self._dial_v.setRange(1, 180); self._dial_v.setValue(30); self._dial_v.setNotchesVisible(True); self._dial_v.setFixedSize(80, 80)
        v_grp.addWidget(self._dial_v, alignment=Qt.AlignCenter)
        v_lbl = QLabel("垂直角度"); v_lbl.setAlignment(Qt.AlignCenter)
        v_grp.addWidget(v_lbl)
        self._spin_v_angle = QDoubleSpinBox(); self._spin_v_angle.setRange(1, 180); self._spin_v_angle.setValue(30); self._spin_v_angle.setFixedWidth(80)
    ...
    def _setup_fw_extra_section(self) -> None:
        ...
        self._spin_speed = QDoubleSpinBox(); self._spin_speed.setRange(0.1, 100); self._spin_speed.setValue(10)
        self._add_row(form, "speed", "速度:", self._spin_speed, 10.0)
        self._spin_inner_speed = QDoubleSpinBox(); self._spin_inner_speed.setRange(0.1, 100); self._spin_inner_speed.setValue(8)
        self._add_row(form, "inner_speed", "内层速度:", self._spin_inner_speed, 8.0)
        self._spin_outer_speed = QDoubleSpinBox(); self._spin_outer_speed.setRange(0.1, 100); self._spin_outer_speed.setValue(10)
        self._add_row(form, "outer_speed", "外层速度:", self._spin_outer_speed, 10.0)
        self._spin_spread = QDoubleSpinBox(); self._spin_spread.setRange(1, 180); self._spin_spread.setValue(15)
        self._add_row(form, "spread", "扩散角:", self._spin_spread, 15.0)
        self._spin_track_count = QSpinBox(); self._spin_track_count.setRange(1, 100); self._spin_track_count.setValue(1)
        self._add_row(form, "track_count", "轨迹数:", self._spin_track_count, 1)
        self._spin_radius = QDoubleSpinBox(); self._spin_radius.setRange(0.1, 100); self._spin_radius.setValue(5); self._spin_radius.setSingleStep(0.5)
        self._add_row(form, "radius", "半径:", self._spin_radius, 5.0)
        self._spin_radial_speed = QDoubleSpinBox(); self._spin_radial_speed.setRange(0.1, 50); self._spin_radial_speed.setValue(3); self._spin_radial_speed.setSingleStep(0.5)
    ...
    def _show_w(self, key: str) -> None:
        if key in self._w:
            self._w[key][0].show(); self._w[key][1].show()
            if self._w[key][2]: self._w[key][2].show()

    def _hide_w(self, key: str) -> None:
        if key in self._w:
            self._w[key][0].hide(); self._w[key][1].hide()
    ...
    def _load_tf_full(self, e) -> None:
        """轨迹烟花完整参数."""
        self._hide_all()
        self._spin_pos_x.setReadOnly(False); self._spin_pos_y.setReadOnly(False); self._spin_pos_z.setReadOnly(False)
        self._spin_end_x.setReadOnly(False); self._spin_end_y.setReadOnly(False); self._spin_end_z.setReadOnly(False)
        self._btn_pos_select.show()
        self._group_common.show()
        self._group_pos.show(); self._group_pos.setTitle("起始位置")
        self._group_endpos.show(); self._group_endpos.setTitle("结束位置")
        self._group_traj_color.show(); self._group_traj_extra.show()
        self._group_fw_inner.show(); self._group_fw_outer.show()
        self._group_fw_angle.show(); self._group_fw_extra.show()
        # 填值
        self._spin_pos_x.setValue(e.start_position.x); self._spin_pos_y.setValue(e.start_position.y); self._spin_pos_z.setValue(e.start_position.z)
        self._spin_end_x.setValue(e.mid_position.x); self._spin_end_y.setValue(e.mid_position.y); self._spin_end_z.setValue(e.mid_position.z)
        ...
        self._color_traj_start.set_color(e.traj_color.start); self._color_traj_end.set_color(e.traj_color.end)
        self._spin_k.setValue(e.k); self._spin_m0.setValue(e.m0); self._spin_rho.setValue(e.rho)
        ...
        self._color_inner_start.set_color(e.inner_color.start); self._color_inner_end.set_color(e.inner_color.end)
        ...
        self._color_outer_start.set_color(e.outer_color.start); self._color_outer_end.set_color(e.outer_color.end)
        self._spin_h_angle.setValue(e.horizontal_angle); self._spin_v_angle.setValue(e.vertical_angle)
        self._spin_speed.setValue(e.speed); self._spin_inner_speed.setValue(e.inner_speed); self._spin_outer_speed.setValue(e.outer_speed)
        self._spin_spread.setValue(e.spread_angle); self._spin_track_count.setValue(e.track_count)
    ...
    def _load_tf_fw_only(self, e) -> None:
        ...
        self._hide_all()
        self._group_common.show()
        self._group_pos.show(); self._group_pos.setTitle("爆炸中心（由轨迹终点决定）")
        self._group_fw_inner.show(); self._group_fw_outer.show()
        self._group_fw_angle.show(); self._group_fw_extra.show()
        self._spin_pos_x.setValue(e.mid_position.x if e.mid_position else 0); self._spin_pos_y.setValue(e.mid_position.y if e.mid_position else 0); self._spin_pos_z.setValue(e.mid_position.z if e.mid_position else 0)
        self._spin_pos_x.setReadOnly(True); self._spin_pos_y.setReadOnly(True); self._spin_pos_z.setReadOnly(True)
    ...
    def _load_trajectory(self, e: TrajectoryElement) -> None:
        ...
        self._spin_pos_x.setValue(e.start_position.x); self._spin_pos_y.setValue(e.start_position.y); self._spin_pos_z.setValue(e.start_position.z)
        self._spin_end_x.setValue(e.end_position.x); self._spin_end_y.setValue(e.end_position.y); self._spin_end_z.setValue(e.end_position.z)
        idx = self._combo_traj_type.findData(e.traj_type)
        if idx >= 0: self._combo_traj_type.setCurrentIndex(idx)
        self._chk_traj_gradient.setChecked(e.traj_color.use_gradient)
        self._color_traj_end.setEnabled(e.traj_color.use_gradient)
        ...
        self._spin_k.setValue(e.k); self._spin_m0.setValue(e.m0)
        self._spin_rho.setValue(e.rho); self._spin_particle_count.setValue(e.particle_count)
        self._spin_interval.setValue(e.interval_ticks); self._spin_pts_per_tick.setValue(e.points_per_tick)
        self._spin_range_x.setValue(e.range_x); self._spin_range_y.setValue(e.range_y); self._spin_range_z.setValue(e.range_z)
    ...
    def _load_firework(self, e: FireworkElement) -> None:
        ...
        self._spin_pos_x.setValue(e.position.x); self._spin_pos_y.setValue(e.position.y); self._spin_pos_z.setValue(e.position.z)
        ...
        self._spin_speed.setValue(e.speed); self._spin_inner_speed.setValue(e.inner_speed)
        self._spin_outer_speed.setValue(e.outer_speed); self._spin_spread.setValue(e.spread_angle)
        self._spin_track_count.setValue(e.track_count)
        self._spin_radius.setValue(e.radius); self._spin_radial_speed.setValue(e.radial_speed)
    ...
    def _on_traj_extra_changed(self) -> None:
        if isinstance(self._current_element, TrajectoryElement) and not self._loading:
            e = self._current_element
            e.k = self._spin_k.value(); e.m0 = self._spin_m0.value()
            e.rho = self._spin_rho.value(); e.particle_count = self._spin_particle_count.value()
            e.interval_ticks = self._spin_interval.value(); e.points_per_tick = self._spin_pts_per_tick.value()
            e.range_x = self._spin_range_x.value(); e.range_y = self._spin_range_y.value(); e.range_z = self._spin_range_z.value()
    ...
    def _on_fw_extra_changed(self) -> None:
        if isinstance(self._current_element, FireworkElement) and not self._loading:
            e = self._current_element
            e.speed = self._spin_speed.value(); e.inner_speed = self._spin_inner_speed.value()
            e.outer_speed = self._spin_outer_speed.value(); e.spread_angle = self._spin_spread.value()
            e.track_count = self._spin_track_count.value()
            e.radius = self._spin_radius.value(); e.radial_speed = self._spin_radial_speed.value()
