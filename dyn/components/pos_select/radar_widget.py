...
class RadarWidget(QWidget):
	...
	def _paint_stored_points(self, p: QPainter) -> None:
		for pt in self.stored_pix_list:
			...
			s = self.pix_size
			path = QPainterPath();
			path.addRoundedRect(x, y, s, s, 3, 3)
			p.fillPath(path, pt.pix_color)
			p.setPen(QPen(pt.pix_color, 1));
			p.drawText(x + s, y, pt.label)
	...
	def _paint_center(self, p: QPainter) -> None:
		x, y = int(self._gx(0, 0)), int(self._gy(0, 0));
		s = self.pix_size
	...
	def _paint_selected(self, p: QPainter) -> None:
		...
		try:
			x, y = int(self._gx(int(self.selected_point.x), int(self.selected_point.z))), int(self._gy(int(self.selected_point.x),
			                                                                                           int(self.selected_point.z)))
			s = self.pix_size
