...
class DirectionalForm(FwBase):
	...
	def _setup_type_sections(self) -> None:
		...
		self._add_row(form, "direction_count", "方向数:", self._spin_dir_count, default=12)
		...
		self._spin_dir_count.valueChanged.connect(self._on_extra_changed)
	...
	def _load_type_sections(self, elem: FireworkElement) -> None:
		...
		self._spin_dir_count.setValue(elem.direction_count)
	...
	def _on_extra_changed(self) -> None:
		...
		self._element.direction_count = self._spin_dir_count.value()
