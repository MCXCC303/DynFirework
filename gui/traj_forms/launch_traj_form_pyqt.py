...
class LaunchTrajForm(TrajectoryFormBase):
    """基础发射轨迹表单"""
    ...
    def __init__(self, parent=None, style=None):
        ...
        # 计算默认参数
        params = ['结束时间 (Tick)', '轨迹持续时间 (秒)', '粒子质量 (m0)', '空气阻力系数 (k)', '粒子存活时间 (秒)', '粒子密度系数 (rho)']
        defaults = [
    ...
    def process_data(self):
        ...
        # 获取颜色
        start_color, end_color = self.get_color_values()
        ...
        rho = float(self.entries['粒子密度系数 (rho)'].text())
        ...
        validation_map = {
            ...
            '粒子密度系数 (rho)': Validators.positive_number
        }
...
if __name__ == "__main__":
    ...
    window = LaunchTrajForm(style=style)
    window.show()
    sys.exit(app.exec_()) 