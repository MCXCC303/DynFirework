...
class ThickTrajWithIntervalForm(TrajectoryFormBase):
    """粗随机扰动发射轨迹表单"""
    ...
    def process_data(self):
        ...
        thickness = float(self.entries['轨迹厚度'].text())
        particle_count = int(self.entries['粒子数量'].text())
        points_per_tick = int(self.entries['每Tick点数'].text())
        range_x = thickness  # 使用厚度作为x方向范围
        range_y = thickness  # 使用厚度作为y方向范围
        range_z = thickness  # 使用厚度作为z方向范围
        ...
        validation_map = {
            ...
            '轨迹厚度': Validators.positive_number,
...
if __name__ == "__main__":
    ...
    window = ThickTrajWithIntervalForm(style=style)
    window.show()
    sys.exit(app.exec_()) 