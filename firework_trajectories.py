...
def launch_trajectory(end_tick, x0, y0, z0, x1, y1, z1, start_color, end_color, duration, k, m0, lifetime, rho):
    # rho表示一个tick内生成多少个粒子
    vx0, vy0, vz0 = calculate_initial_velocity_bisection(x0, y0, z0, x1, y1, z1, duration, k, m0)  # 计算初始速度
    t = 0
    ...
    while t <= duration:
        ...
        # 计算垂直方向的位移
        y_ = y0 - (m0 * G * t / k) + (vy0 + (m0 * G / k)) * (m0 / k) * (
                1 - math.exp(-k * t / m0))
        ...
        # 添加粒子指令
        shared_functions.add_firework_command(int(n_tick), round(x_, 4), round(y_, 4), round(z_, 4), lifetime * 20,
                                              color_expr)
...
def expanding_trajectory_with_random_offset(end_tick, x0, y0, z0, x1, y1, z1, k, m0, duration, lifetime, interval_ticks,
    ...
    for i in range(num_points):
        ...
        shared_functions.add_thick_spark_command(n_tick, round(initial_x, 4), round(initial_y, 4), round(initial_z, 4),
                                                 vx, vy, vz, lifetime * 20, range_x, range_y, range_z, particle_count)
